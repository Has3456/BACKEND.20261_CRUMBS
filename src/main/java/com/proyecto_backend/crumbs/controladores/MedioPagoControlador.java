...
@RestController
@RequestMapping("api/v1/medios-pago")
public class MedioPagoControlador {
    ...
    public ResponseEntity<?> controaldorGuardar(@PathVariable Integer usuarioId, @RequestBody MedioPago datos) {
        return ResponseEntity.status(HttpStatus.OK).body(servicio.guardar_medio_pago(usuarioId, datos));
    }

    @GetMapping
    public ResponseEntity<?> controaldorListar() {
        return ResponseEntity.status(HttpStatus.OK).body(servicio.listar_medios_pago());
    }

    // control para modificar
    @PutMapping("/{id}")
    public ResponseEntity<?> controaldorModificar(@PathVariable Integer id, @RequestBody MedioPago datos) {
